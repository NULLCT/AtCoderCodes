...
void execution() {
  int n;  cin >> n;
  vector<pair<int, int>> xy(n);  cin >> xy;
  string s;  cin >> s;
  unordered_map<int,int> r, l; // r: ->  l:  <-

  for (int i : Range(n)) {
    if(s[i] == 'R'){ // r
      if(r.find(xy[i].second) != r.end())
        chmin(r[xy[i].second], xy[i].first);
      else
        r[xy[i].second] = xy[i].first;
    }else{ // l
      if(l.find(xy[i].second) != r.end())
        chmax(l[xy[i].second], xy[i].first);
      else
        l[xy[i].second] = xy[i].first;
    }
  }
