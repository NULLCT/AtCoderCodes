...
inline void execution() {
  int n;cin>>n;
  vector<pair<string,bool>> ss(n); // true -> !
  for(auto &i:ss){
    string s;cin>>s;
    if(s[0] == '!'){
      i=pair<string,bool>(s.substr(1,s.size()-1),true);
    }else{
      i=pair<string,bool>(s,false);
    }
  }

  sort(ALL(ss),[](pair<string,bool> l,pair<string,bool> r){
  ...
  for(int i=0;i<n-1;i++){
    if(ss[i].first == ss[i+1].first){
      if((ss[i].second or ss[i+1].second) and (ss[i].second != ss[i+1].second)){
        cout<<ss[i].first<<"\n";
        return;
      }
    }
