...
void execution() {
  ...
  deque<pair<int,int>> cv;
  for(int i:a){
    if(cv.size() != 0 and cv.end()[-1].second >= i-1){
      cv.end()[-1].second = i+w-1;
    }else{
      cv.push_back({i,i+w-1});
    }
  }
  ...
  for(int i:Range(1,cv.size())){
    ans += max((cv[i].first-cv[i-1].second-1+(-1+w))/w,0LL);
    D(((cv[i].first-cv[i-1].second-1+(-1+w))/w));
  }
  cout<<ans<<"\n";
}
