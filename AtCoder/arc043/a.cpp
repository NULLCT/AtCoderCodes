...
void execution() {
  int n,a,b;cin>>n>>a>>b;
  vector<double> s(n);cin>>s;

  int dif = [&](){
    auto e = minmax_element(ALL(s));
    return *e.second-*e.first;
  }();
  if(dif == numeric_limits<int>::infinity()){
    cout<<-1<<"\n";
    return;
  }
  D(dif);

  double conv = static_cast<double>(b)/dif;
  cout<<conv<<" ";
  cout<<a-(accumulate(ALL(s),0.0)*conv/n)<<"\n";
}
