...
inline void execution() {
  ...
  for (int i = 0; i < m - 1; i++) {
    if(a[i+1] - a[i] != 1)
      minst = min(minst, a[i + 1] - a[i] - 1);
  }
  if (a[a.size() - 1] != n - 1)
    minst = min(minst, (n - 1) - a[a.size() - 1]);
  ...
  if(a[0] != 0)
    ans += ceil(a[0] / static_cast<long double>(minst));
  for (int i = 0; i < m - 1; i++) {
    if(a[i+1] - a[i] != 1)
      ans += ceil((a[i + 1] - a[i] - 1) / static_cast<long double>(minst));
  }
  if(a[a.size()-1] != n-1)
    ans += ceil((n - a[a.size() - 1] - 1) / static_cast<long double>(minst));
