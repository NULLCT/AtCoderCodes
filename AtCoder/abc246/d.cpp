...
#define DD(var) for(const auto &i:var) D(i);
#else
#define D(var) ;
#define DD(var) ;
#endif
...
constexpr array<int,4> dx{-1,-1,1,1};
constexpr array<int,4> dy{1,-1,-1,1};
...
int mpow(int a, int n, int mod=LONG_LONG_MAX) {
  int res = 1;
  while (n > 0) {
    if (n & 1)
      res = res * a % mod;
...
void execution() {
  ...
  for(int a=0;a<1000000;a++){
    int res = dichotomy(-1,1000000LL,[&](int b){
          return n <= a*a*a + a*a*b + a*b*b + b*b*b;
        });
    chmin(ans,a*a*a + a*a*res + a*res*res + res*res*res);
  }
  cout<<ans<<"\n";
}
