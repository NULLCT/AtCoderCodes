...
void solve() {
  int n;cin>>n;
  pair<int,int> st,ed;cin>>st>>ed;

  deque<pair<pair<int,int>,int>> cs(n);cin>>cs;
  cs.push_front({st,0});
  cs.push_back({ed,0});
  n+=2;
  ...
  for(int i:range(n)){
    for(int j:range(i+1,n)){
      int x0 = cs[i].first.first;
      int y0 = cs[i].first.second;
      int x1 = cs[j].first.first;
      int y1 = cs[j].first.second;

      int dist = (mop(x0-x1,2)+mop(y0-y1,2));
      if(not (mop(cs[i].second + cs[j].second,2) < dist) and not (mop(cs[i].second-cs[j].second,2) > dist)){
        uf.merge(i,j);
      }
    }
  }

  cout<<(uf.isSame(0,n-1) ? "Yes" : "No")<<"\n";
}
