...
void solve() {
  array<array<char,9>,9> s;cin>>s;
  int ans = 0;
  for(int y0=0;y0<9;y0++){
    for(int x0=0;x0<9;x0++){
      for(int y1=y0;y1<9;y1++){
        for(int x1=x0+1;x1<9;x1++){
          if(not(x0 == x1 and y0 == y1)){
            ...
            c = {x0+dy,y0-dx};
            d = {x1+dy,y1-dx};
            if(0 <= c.first and c.first < 9 and
                0 <= c.second and c.second < 9){
              if(0 <= d.first and d.first < 9 and
                  0 <= d.second and d.second < 9){
                if(
                    s[a.first][a.second] == '#' and
                    s[b.first][b.second] == '#' and
                    s[c.first][c.second] == '#' and
                    s[d.first][d.second] == '#'
                    ){
                  D(a);
                  D(b);
