...
inline void execution() {
...

  {
    auto ax = a;
    sort(ALL(ax));
    auto itr = unique(ALL(ax));
    ax.erase(itr,ax.end());

    for(int i:Range(n)){
      anses[i].first = (lower_bound(ALL(ax),a[i])-ax.begin())+1;
    }
  }

  {
    auto bx = b;
    sort(ALL(bx));
    auto itr = unique(ALL(bx));
    bx.erase(itr,bx.end());

    for(int i:Range(n)){
      anses[i].second = (lower_bound(ALL(bx), b[i]) - bx.begin())+1;
    }
  }
