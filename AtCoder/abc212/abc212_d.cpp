...
inline void execution() {
  int q;cin>>q;
  multiset<pair<int,int>> st; // <num, xでどれだけ前に引かれたか>
  ...
  while(q--){
    ...
    if(p == 1){
      int x;cin>>x;
      st.insert(make_pair(x-*prev(sums.end()),*prev(sums.end())));
    }else
    if(p == 2){
      int x;cin>>x;
      sums.push_back(*prev(sums.end()) + x);
    }else
    if(p == 3){
      auto ptr = st.begin();
      cout<<(*ptr).first + (*ptr).second + *prev(sums.end()) - *lower_bound(ALL(sums),(*ptr).second)<<"\n";
      st.erase(ptr);
    }
  }
}
