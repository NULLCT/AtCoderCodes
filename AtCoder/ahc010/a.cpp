...
ostream &operator<<(ostream &_ostr, const array<T,N> &_v);
template <class T>
...
void scanRails(array<array<char,N>,N> &a){
  UnionFind uf(N*N);
  vector<vector<vector<pair<int,int>>>> c(N,vector<vector<pair<int,int>>>(N));
  for(int y:range(N)){
    for(int x:range(N)){
      if(a[y][x] == 0){
        c[y][x].push_back({y-1,x});
        c[y][x].push_back({y,x-1});
      }else if(a[y][x] == 1){
        c[y][x].push_back({y,x-1});
        c[y][x].push_back({y+1,x});
      }else if(a[y][x] == 2){
  ...
  for(int y:range(N)){
    for(int x:range(N)){
      if(x < N-1){
        if(find(ALL(c[y][x+1]),pair(x,y)) != c[y][x+1].end()){
          uf.merge(y*N+x,y*N+x+1);
          D(1);
        }
      }
      if(y < N-1){
        if(find(ALL(c[y+1][x]),pair(x,y)) != c[y+1][x].end()){
