  ...
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      if (S[i][j] == '#')
        continue;
      if (j > 0)
        (dp[i][j] += yoko[i][j - 1]) %= MOD;
      if (i > 0)
        (dp[i][j] += tate[i - 1][j]) %= MOD;
      if (i > 0 && j > 0)
        (dp[i][j] += naname[i - 1][j - 1]) %= MOD;
      yoko[i][j] = tate[i][j] = naname[i][j] = dp[i][j];
      if (j > 0)
        (yoko[i][j] += yoko[i][j - 1]) %= MOD;
      if (i > 0)
        (tate[i][j] += tate[i - 1][j]) %= MOD;
      if (i > 0 && j > 0)
        (naname[i][j] += naname[i - 1][j - 1]) %= MOD;
    }
  }
