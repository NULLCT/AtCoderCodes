...
pair<int,int> mis(int x, int y){
  int div = gcd(x,y);
  return make_pair(x/div, y/div);
}

void execution() {
  int n;cin>>n;
  vector<pair<int,int>> ps(n);cin>>ps;
  ...
  auto f = [&st](pair<int,int> i){
    auto [x,y] = i;
    int div = gcd(abs(x),abs(y));
    st.insert(make_pair(x/div,y/div));
  };

  for(int i=0;i<n;i++){
    for(int j=i+1;j<n;j++){
      auto dif = make_pair(ps[i].first-ps[j].first,ps[i].second-ps[j].second);
      f(dif);
      dif = make_pair(ps[j].first-ps[i].first,ps[j].second-ps[i].second);
      f(dif);
    }
  }
