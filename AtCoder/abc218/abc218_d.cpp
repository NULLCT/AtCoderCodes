...
vector<vector<char>> disolve(const vector<vector<char>> &s, int n) {
  pair<int,int> bg,ed;
...
END3:
  ...
  vector<vector<char>> res(ed.second-bg.second+1, vector<char>(ed.first-bg.first+1));
  for(int i=bg.first ;i <= ed.first; i++){
    for(int j=bg.second ; j <= ed.second; j++){
      res[j-bg.second][i-bg.first] = s[j][i];
    }
  }
...
vector<vector<char>> turnR(const vector<vector<char>> &s){
  vector<vector<char>> res(s[0].size(),vector<char>(s.size()));
...
void execution() {
  ...
  for(int a=0;a<n;a++){
    for(int b=a+1;b<n;b++){
      if(p[a].first != p[b].first) continue;
      for(int c=b+1;c<n;c++){
        if(p[a].second != p[c].second) continue;
        for(int d=c+1;d<n;d++){
          if(p[d].first == p[c].first and p[d].second == p[b].second)
            ans++;
        }
      }
    }
