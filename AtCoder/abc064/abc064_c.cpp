...
int main() {
  ...
  REP(i,n){
    if(a[i]<400) cnt[0]=1;
    else if(a[i]<800) cnt[1]=1;
    else if(a[i]<1200) cnt[2]=1;
    else if(a[i]<1600) cnt[3]=1;
    else if(a[i]<2000) cnt[4]=1;
    else if(a[i]<2400) cnt[5]=1;
    else if(a[i]<2800) cnt[6]=1;
    else if(a[i]<3200) cnt[7]=1;
    else x++;
  }
  int ans=0;
  REP(i,8)	ans+=cnt[i];
  cout<<max(1,ans)<<" "<<ans+x<<endl; 
  return 0;
