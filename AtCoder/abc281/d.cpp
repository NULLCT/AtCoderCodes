...
void solve() {
  ...
  vector dp(n,vector<vector<int>>(n+1,vector<int>(d,0)));
  vector trs(n,vector<vector<bool>>(n+1,vector<bool>(d,false)));
  trs[0][1][a[0]%d] = true;
  trs[0][0][0] = true;
  dp[0][1][a[0]%d] = a[0];
  //dp[a][b][c] aまででbコ使って%dがcになる
  for(int x=0;x<n-1;x++){
    for(int y=0;y<=x+1;y++){
      for(int z:range(d)){
        if(trs[x][y][z])trs[x+1][y][z] = true;
        chmax(dp[x+1][y][z],dp[x][y][z]);
        int add = dp[x][y][z] + a[x+1];
        if(trs[x][y][z])trs[x+1][y+1][add%d]=true;
        chmax(dp[x+1][y+1][add%d],add);
      }
    }
  }
  for(int i=0;i<n;i++){
