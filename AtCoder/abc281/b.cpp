...
void solve() {
  ...
  if (s.size() == 8) {
    if ((int)'A' <= (int)s[0] and (int) s[0] <= (int)'Z') {
      if ((int)'A' <= (int)s.back() and (int) s.back() <= (int)'Z') {
        D("a")
        for (int i : range(1, s.size() - 1)) {
          if (not ((int)'0' <= (int)s[i] and (int) s[i] <= (int)'9')) {
            cout << "No\n";
            return;
          }
        }
        int n = stoll(s.substr(1,6));
        if(100000 <= n and n <= 999999){
          cout << "Yes\n";
          return;
        }
      }
