...
inline void execution() {
  int n, a;  cin >> n >> a;
  string s;  cin>>s;  s='x'+s+'x';

  list<int> shaps;
  int firstshp=-1;
  int firstshpinshaps=-1;
  for (int i = 1; i < n+1; i++) {
    if (s[i] == '#') {
      shaps.push_back(i);
      if (firstshp == -1 and a <= i) {
        firstshp=i;
        firstshpinshaps=shaps.size()-1;
      }
    }
  }

  if (firstshpinshaps == -1) {
    firstshpinshaps = shaps.size();
  }


  int ans=0;
  int pos=a;
  int shapcnt=shaps.size();
  bool tr=true;
  auto cache = next(shaps.begin(), firstshpinshaps);
  ...
  while (true) {
    if (tr) {
      ...
      cache--;
      shaps.erase(next(cache,1));

      tr = !tr;
    } else {
      if (cache == prev(shaps.begin(),1)) {
        ans+=(pos)*2;
        cache++;
