...
void solve() {
  ...
  valarray<valarray<char>> a(valarray<char>(w),h);cin>>a;
  vector<vector<mint>> dp(h,vector<mint>(w,0LL));
  valarray<valarray<bool>> trs(valarray<bool>(false,w),h);
  ...
  while(not que.empty()){
    ...
    if(trs[q.first][q.second])
      continue;
    trs[q.first][q.second] = true;

    for(int i=0;i<2;i++){
      if(q.first+dy[i] < h and q.second+dx[i] < w){
        if(a[q.first+dy[i]][q.second+dx[i]] == '.'){
          que.push({q.first+dy[i], q.second+dx[i]});
          dp[q.first+dy[i]][q.second+dx[i]] += dp[q.first][q.second];
        }
      }
    }
  }
