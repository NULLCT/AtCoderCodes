...
void solve() {
  int N,W;cin>>N>>W;
  valarray<int> w(N),v(N);
  for(int i:range(N))
    cin>>w[i]>>v[i];


  constexpr int max = 1000000+2;
  valarray<valarray<int>> dp(valarray<int>(INT_MAX,max),N); // dp[i][j] -> i番目まで選んだとき価値jの重さ最小
  dp[0][0] = 0;
  dp[0][v[0]] = w[0];
  ...
  for(int i=1;i<N;i++){
    for(int j=0;j<max;j++){
      ...
      if(j+v[i] < max)
        chmin(dp[i][j+v[i]],dp[i-1][j]+w[i]);
    }
  }
  int ans = max-1;
  while(end(dp)[-1][ans]>W)ans--;
  cout<<ans<<"\n";
}
