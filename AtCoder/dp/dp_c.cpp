...
void execution() {
  int n;cin>>n;
  vector<vector<int>> d(n,vector<int>(3));cin>>d;
  ...
  for(int i=1;i<n;i++){
    chmax(dp[i][0],d[i][0]+max(dp[i-1][1],dp[i-1][2]));
    chmax(dp[i][1],d[i][1]+max(dp[i-1][0],dp[i-1][2]));
    chmax(dp[i][2],d[i][2]+max(dp[i-1][0],dp[i-1][1]));
  }

  cout<<*max_element(ALL(dp.end()[-1]))<<"\n";
}
