...
void execution() {
  ...
  cin >> n >> W;
  vector<int> w(n),v(n);
  for(int i:Range(n)) cin >> w[i] >> v[i];
  vector<vector<int>> dp(n+1,vector<int>(100010,INT_MAX));
  dp[0][0] = 0;
  //dp[i][j]:i-1番目までで，価値がj以上になるときの重さの総和の最小値
  for(int i:Range(n)){
      for(int j:Range(100010)){
