...
//--------------------------------------------------------------
void f(vector<int> boxes,const vector<pair<int,int>> &wv){
  int ans = 0;
...
inline void execution() {
  int n,m,q;cin>>n>>m>>q;
  vector<pair<int,int>> wv(n);for(int i:Range(n))cin>>wv[i].first>>wv[i].second; // pair<weight,value>
  vector<int> x(m);for(auto &&i:x)cin>>i;

  sort(ALL(wv), [](auto &l, auto &r) {
