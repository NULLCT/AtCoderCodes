...
vector<pair<int,int>> makeMobableTable(int m){
...
void solve() {
  ...
  auto movable = makeMobableTable(m);
  ...
  while(not que.empty()){
    ...
    for(auto &mov:movable){
      auto ok = [&](int x,int y){
        return (0 <= x and x <= n-1 and 0 <= y and y <= n-1);
      };

      if(ok(mov.first + q.first, mov.second + q.second)){
        if(chmin(dp[mov.first+q.first][mov.second+q.second],dp[q.first][q.second]+1)){
          que.push_back({mov.first+q.first,mov.second+q.second});
        }
      }
    }
  }
