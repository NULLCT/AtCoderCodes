...
inline void execution() {
  int n,m;cin>>n>>m;
  Graph grhraw(n),grhrev(n);
  REP(i,m){
    int a,b,c;cin>>a>>b>>c;
    grhraw.add_edge(a-1,b-1,c);
    grhrev.add_edge(b-1,a-1,c);
  }


  REP(i,n){
    grhraw.dijkstra(i);grhrev.dijkstra(i);

    int ans=LONG_LONG_MAX;
    REP(j,n){
      if(grhraw.d[j] != 0 and (grhraw.d[j] != INT_MAX and grhrev.d[j] != INT_MAX)){
        ans = min(grhraw.d[j]+grhrev.d[j],ans);
      }
    }

    for(auto &j:grhraw.G[i]){
      if(i == j.to){
        ans=min(j.cost,ans);
      }
    }
