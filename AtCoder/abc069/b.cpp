...
class Range {
  ...
  const T m_stride;
public:
  Range(T begin, T end, T stride) : m_value(begin), m_end(end), m_stride(stride) {}
...
template <typename T>
Range<T> r(T end) {
  return {static_cast<T>(0), end, static_cast<T>(1)};
}
template <typename T>
Range<T> r(T begin, T end) {
  return {begin, end, static_cast<T>(1)};
}
template <typename T>
Range<T> r(T begin, T end, T stride) {
...
void execution() {
  string s;cin>>s;
  cout<< s[0] + to_string(s.size()-2) + s.end()[-1]<<"\n";
}
