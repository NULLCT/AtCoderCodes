...
public:
  ...
  vector<vector<int>> warshallfloyd() { // O(v^3)
  ...
  vector<int> dijkstra(int s) { // O(E + VlogV) N竓・E
  ...
  vector<int> bellmanfood(int s) { // O(|V||E|) R竓・E
    vector<int> d(n, INT_MAX);
    d[s] = 0;
    for (int _ = 0; _ < n; _++) {
      bool upd = false;
...
void execution() {
  ...
  D(sum);
  int ans = 0;
  for(int i:Range(sum.size()-1)){
    chmax(ans,abs(sum[i]-sum[i+1]));
  }
  cout<<ans<<"\n";
}
