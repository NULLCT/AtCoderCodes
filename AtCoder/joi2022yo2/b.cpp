...
constexpr array<int, 4> dx{0, 1, 0, -1}, dy{1, 0, -1, 0};
...
void solve() {
  ...
  while(not que.empty()){
    ...
    for(int i=0;i<4;i++){
      ...
      if(0 <= y and y < h and
         0 <= x and x < w){
        if(ss[q.first][q.second] != ss[y][x] and chmin(dp[y][x],dp[q.first][q.second]+1)){
          que.push({y,x});
        }
      }
    }
  }
  cout<<(dp[h-1][w-1] == LLONG_MAX ? -1 : dp[h-1][w-1])<<"\n";
}
