...
void execution() {
  ...
  vector<vector<int>> dp(h,vector<int>(w,INT_MIN));
  ...
  while(not que.empty()){
    ...
    if(p.first+1 < w and c[p.second][p.first+1] == '.'){
      if(dp[p.second][p.first+1]+1 < dp[p.second][p.first]){
        que.push(pair(p.first+1,p.second));
        chmax(dp[p.second][p.first+1],dp[p.second][p.first]+1);
      }
    }
    if(p.second+1 < h and c[p.second+1][p.first] == '.'){
      if(dp[p.second+1][p.first]+1 < dp[p.second][p.first]){
        que.push(pair(p.first, p.second+1));
        chmax(dp[p.second+1][p.first],dp[p.second][p.first]+1);
      }
    }
  }
