...
void execution() {
  ...
  set<pair<int,int>> cnc;
  for(int i:Range(m)){
    int a,b;cin>>a>>b;
    a--;b--;
    ga[a].insert(b);
    ga[b].insert(a);
    cnc.insert(pair(min(a,b),max(a,b)));
  }
  ...
  do{
    ...
    for(int i:Range(n)){
      for(int j=i+1;j<n;j++){
        if(not (gt[i].find(j) != gt[i].end() and ga[p[i]].find(p[j]) != ga[p[i]].end()) and ga[p[j]].find(p[i]) != ga[p[j]].end()){
          goto END;
        }
      }
    }
