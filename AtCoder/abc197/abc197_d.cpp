...
inline void execution() {
  int n;cin>>n;
  pair<long double,long double> b,e;cin>>b.first>>b.second>>e.first>>e.second;

  pair<long double,long double> mid = {(b.first+e.first)/2,(b.second+e.second)/2};

  PRINT(mid);

  long double direct = atan2((b.second-mid.second),(b.first-mid.first));
  direct += (M_PI/180)*(360.0/n);
  PRINT(direct);

  long double distance = sqrt(pow(mid.first-b.first,2)+pow(mid.second-b.second,2));
  PRINT(distance);

  cout<<(cos(direct)*distance)+mid.first<<" "<<(sin(direct)*distance)+mid.second<<"\n";
}
