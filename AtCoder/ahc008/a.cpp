...
void execution() {
  int n;cin>>n;
  vector<vector<int>> isblock(30,vector<int>(30,0)); // (x,y) 0 -> free : blocked decrease~~
  vector<Pet> pets(n);
  for(auto &i:pets){
    cin>>i.x>>i.y>>i.type;i.x--;i.y--;
    isblock[i.x][i.y]++;
  }
  int m;cin>>m;
  vector<pair<int,int>> hums(m);
  for(auto &i:hums){
    cin>>i;i.first--;i.second--;
    isblock[i.first][i.second];
  }
