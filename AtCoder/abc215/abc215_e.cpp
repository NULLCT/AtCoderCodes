...
inline void execution() {
  ...
  vector<vector<vector<ModNum<998244353>>>> dp(1010,vector<vector<ModNum<998244353>>>(1 << 10,vector<ModNum<998244353>>(11)));

  dp[0][0][10] = 1;

  for(int i:Range(n)) for(int msk:Range(1 << 10)) for(int lst:Range(11)){
    if(not (dp[i][msk][lst] == 0)){
      dp[i+1][msk][lst] += dp[i][msk][lst];

      int nxt = s[i] - 'A';
      if((msk & (1 << nxt)) && lst != nxt)
        continue;
      dp[i+1][msk | (1 << nxt)][nxt] += dp[i][msk][lst];
    }
  }

  ModNum<998244353> ans = 0;
  for(int msk:Range(1 << 10)) for(int lst:Range(10))
    ans += dp[n][msk][lst];
