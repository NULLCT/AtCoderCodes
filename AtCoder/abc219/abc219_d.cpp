...
void execution() {
  ...
  for (int x = 0; x <= X; x++) {
    for (int y = 0; y <= Y; y++) {
      for (int i = 0; i < n; i++) {
        dp[min(x + p[i].first, X)][min(y + p[i].second, Y)][i + 1] = min(dp[min(x + p[i].first, X)][min(y + p[i].second, Y)][i + 1], dp[x][y][i] + 1);
        dp[x][y][i + 1] = min(dp[x][y][i + 1], dp[x][y][i]);
      }
    }
  }

  cout<<(dp[X][Y][n] == INT_MAX ? -1 : dp[X][Y][n])<<"\n";
}
