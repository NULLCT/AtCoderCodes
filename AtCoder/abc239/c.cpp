...
void execution() {
  pair<int,int> a,b;cin>>a>>b;

  set<pair<int,int>> anses;

  auto f = [&anses](pair<int,int> p){
    auto chk = [&anses,&p](int x,int y){
      if(anses.find({p.first+x,p.second+y}) == anses.end()){
        anses.insert({p.first+x,p.second+y});
      }else{
        cout<<"Yes\n";
        exit(0);
      }
