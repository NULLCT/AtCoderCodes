...
void execution() {
  ...
  vector<pair<int,int>> g(m);cin>>g;
  for(auto &i:g){
    i.first--;
    i.second--;
  }

  vector<vector<vector<ModInt<998244353>>>> dp(k+1,vector<vector<ModInt<998244353>>>(n,vector<ModInt<998244353>>(2,0)));
  dp[0][s][0] = 1;
  for(int i:Range(k)){
    for(const auto &[u,v] : g) for(int l:Range(2)){
      dp[i+1][v][l ^ (v==x)] += dp[i][u][l];
      dp[i+1][u][l ^ (u==x)] += dp[i][v][l];
    }
  }
  cout<<dp[k][t][0]<<"\n";
}
