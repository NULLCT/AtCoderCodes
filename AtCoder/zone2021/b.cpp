...
//--------------------------------------------------------------
pair<long double,long double> getLine(pair<long double,long double> p1,pair<long double,long double> p2){
  // y = ax + b
  // y = ax + b
  ...
  long double a = (double)ydiff / xdiff;
  long double b = p1.second - p1.first * a;
...
inline void execution() {
  ...
  long double ans = 0;
  while(n--){
    int dis,hei;cin>>dis>>hei;
    auto line = getLine(make_pair(d,h),make_pair(dis,hei));
    ans = max(ans,line.second);
    PRINT(line.second);
  }
