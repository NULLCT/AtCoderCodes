...
inline void execution() {
  string s;cin>>s;
  vector<int> n = {s[0]-'0',s[1]-'0',s[2]-'0',s[3]-'0'};
  vector<char> ans(3);

  function<void(int,int,vector<char>)> fun;
  fun = [&](int num,int sum,vector<char> ss){
    if(num == 4){
      if(sum == 7){
        ans = ss;
      }
    ...
    ss[num-1] = '+';
    fun(num+1,sum+n[num],ss);
    ss[num-1] = '-';
    fun(num+1,sum-n[num],ss);
  };
  
  fun(1,n[0],vector<char>(3));

  cout<<n[0]<<ans[0]<<n[1]<<ans[1]<<n[2]<<ans[2]<<n[3]<<"=7\n";
}
