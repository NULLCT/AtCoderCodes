...
inline void execution() {
  ...
  for (int first = 0; first < n; first++) {
    for (int second = first+1; second < n; second++) {
      for (int therd = second+1; therd < n; therd++) {
        vector<pair<int,int>> xy(3);

        xy[0].first=pos[first].first;
        xy[1].first=pos[second].first;
        xy[2].first=pos[therd].first;

        xy[0].second=pos[first].second;
        xy[1].second=pos[second].second;
        xy[2].second=pos[therd].second;

        sort(ALL(xy),compare_by_b);

        if ((xy[2].first - xy[0].first) * (xy[1].second - xy[0].second) ==
            (xy[1].first - xy[0].first) * (xy[2].second - xy[0].second)) {
          cout<<"Yes\n";
          return;
        }
