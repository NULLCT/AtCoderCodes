...
inline void execution() {
  int n,c;cin>>n>>c;
  vector<pair<int,int>> tl; // pair<time,money>

  for(int i=0;i<n;i++){
    int a,b,c;cin>>a>>b>>c;
    tl.emplace_back(make_pair(a,c));
    tl.emplace_back(make_pair(b+1,-c));
  }

  sort(ALL(tl),[](pair<int,int> &l,pair<int,int> &r){
  ...
  for(int i=0;i<tl.size();i++){
    if(i != 0){
      ans+=min(c,husai)*(tl[i].first-tl[i-1].first);
    }
    husai+=tl[i].second;
  }
  cout<<ans<<"\n";
