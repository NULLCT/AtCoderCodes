...
inline void execution() {
  ...
  while(true){
    PRINT(a);
    if(a.size() == 2){
      cout<<(a[0].second < a[1].second ? a[0].first : a[1].first)<<"\n";
      return;
    }
    vector<pair<int,int>> ary(LEN(a)/2);
    for(int i=0;i<a.size();i+=2){
      ary[i/2]=(a[i].second < a[i+1].second ? make_pair(a[i+1].first,a[i+1].second) : make_pair(a[i].first,a[i].second));
    }
    a=ary;
  }
}
