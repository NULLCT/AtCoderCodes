...
inline void execution() {
  ...
  vector<int> a(n);
  vector<int> x(m), y(m);
  vector<pair<int, int>> load(m);

  for(auto &i:a)cin>>i;
  for(auto i:irange(m))cin >> x[i] >> y[i];

  for(auto i:irange(m)) load[i] = {x[i],y[i]};
  sort(ALL(load));

  //町iにいる時の利益をdp[i]とする(未購入の場合は0)
  vector<int> dp(n, -1001001000);

  for(auto i:irange(m)){
    if (dp[load[i].first - 1] >= 0) {
      //移動前の値がプラスの時だけ加算
      dp[load[i].second - 1] = max(dp[load[i].second - 1], a[load[i].second - 1] - a[load[i].first - 1] + dp[load[i].first - 1]);
    } else {
      //移動前がマイナスの時は最初の移動とする
      dp[load[i].second - 1] = max(dp[load[i].second - 1], a[load[i].second - 1] - a[load[i].first - 1]);
    }
  }
