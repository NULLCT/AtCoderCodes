...
inline void execution() {
  ...
  vector<pair<int,int>> dp(101,make_pair(INT_MAX,INT_MAX));
  ...
  if(a <= b){
    for(int i=a;i<=b;i++){
      if(i == a){
        ...
        dp[i].first=min({dp[i-1].first+y,dp[i-1].second+x,dp[i].second+x});
        dp[i].second=min(dp[i].first+x,dp[i-1].second+y);
      }
    }
    cout<<dp[b].second<<"\n";
  }else{
    for(int i=a;i>=b;i--){
      if(i == a){
        ...
        dp[i].second=min({dp[i+1].first+x,dp[i+1].second+y,dp[i].second+x});
        dp[i].first=min(dp[i+1].first+y,dp[i].second+x);
      }
    }
    cout<<dp[b].second<<"\n";
  }
